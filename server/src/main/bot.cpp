...
void Bot::update_player() {
  ...
  lua_newtable(this->state);
  // printf("par(%f,%f)", player.get_position().getX(),
  // player.get_position().getY()); printf("----------");
  if (player_goal != nullptr) {
  ...
  if (player_goal != nullptr) {
    ...
    //  (own_position, players.at(id_at_players)), player.get_position().getX(),
    ...
    // printf("-.%d.-", checker.get_knife_range_collides_player_id(own_position,
    // players.at(id_at_players)));
