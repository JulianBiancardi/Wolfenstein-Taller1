...
Bot::Bot(CollisionChecker &checker, Player &player,
         std::unordered_map<int, Player> &players)
    : checker(checker), player(player), players(players), player_goal(nullptr) {
...
void Bot::kill_actions() {
  ...
  // printf(" X: %f Y:%f   ",
  // player.get_position().getX(),player.get_position().getY()); printf("NUM:
  // %d", checker.get_collides_player_id(own_position,
  // players.at(id_at_players)));
...
void Bot::move_actions() {
  ...
  if (checker.can_move(new_point, players.at(id_at_players))) {
    // printf("SE PUEDE MOVER? |%d|",  checker.can_move(new_point,
    // players.at(id_at_players)));
    player.set_position(Point(x, y));
  } else {
    // TODO CHECKEAR QUE ESTE EN EL RANGO DE ALCANCE
    // printf("NO %d",     checker.get_collides_player_id(new_point,
    // players.at(id_at_players)));
    if (player_goal != nullptr &&
        checker.get_collides_player_id(new_point, players.at(id_at_players)) ==
            player_goal->get_id()) {
  ...
  // printf("
  // Player:[%f][%f]",player.get_position().getX(),player.get_position().getY());
  // printf("\n");
}
...
void Bot::set_id_at_players(int id) { this->id_at_players = id; }
