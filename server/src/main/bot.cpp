...
void Bot::move_actions(){
  ...
  if (checker.can_move(new_point, players.at(id_at_players))){
    //printf("SE PUEDE MOVER? |%d|",  checker.can_move(new_point, players.at(id_at_players)));
    player.set_position(Point(x,y));
  }else{
    //TODO CHECKEAR QUE ESTE EN EL RANGO DE ALCANCE
    //printf("NO %d",     checker.get_collides_player_id(new_point, players.at(id_at_players)));
    if (player_goal != nullptr &&
        checker.get_collides_player_id
