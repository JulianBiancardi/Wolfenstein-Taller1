...
Player& EventHandlerBuilder::get_doer(
    const packet_t& event, std::unordered_map<int, Player>& players) {
...
EventHandler* game_over_handler() { return new GameOverHandler(); }

EventHandler* EventHandlerBuilder::build(
    const packet_t& event, std::unordered_map<int, Player>& players) {
  switch (event.type) {
    case MOVE_PACKET:
      return move_handler(get_doer(event, players), event);
    case ROTATE_PACKET:
      return rotate_handler(get_doer(event, players), event);
    case DOOR_PACKET:
      return door_interaction_handler(event);
    case CHANGE_GUN_PACKET:
      return change_gun_handler(get_doer(event, players), event);
    case DAMAGE_PACKET: {
    ...
    default:
      throw -1;  // TODO Throw ours exception
  }
}
