...
Shooter::Shooter(Point origin, double angle, Map &game_map):
    angled_position(origin, angle), map(game_map)
{}

void Shooter::shoot(Gun &gun) {
  Randomizer<float> randomizer;
  float random_angle = randomizer.operator()(angled_position.get_angle()-M_PI/12,
