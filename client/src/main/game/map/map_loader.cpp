...
void MapLoader::add_item(const Ray& position, unsigned int resource_id) {
  ...
  switch (resource_id) {
    ...
    case ROCKET_LAUNCHER:
      new_item =
          std::make_shared<RocketLauncherItem>(position, resource_id,
                                               next_id++);
      break;
    case FOOD:
    ...
    case BLOOD:
      ...
      break;
    case BULLETS:
      new_item = std::make_shared<Bullets>(position, resource_id,
                                           next_id++);
      break;
    case CROSS:
      new_item = std::make_shared<Cross>(position, resource_id, next_id++);
      break;
    case CUP:new_item = std::make_shared<Cup>(position, resource_id, next_id++);
      break;
    case CHEST:
    ...
    case CROWN:
      ...
      break;
    case KEY:new_item = std::make_shared<Key>(position, resource_id, next_id++);
      break;
  }
...
void MapLoader::load_map(const std::string& map_name) {
  ...
  for (iter = objects.begin(); iter != objects.end(); iter++) {
    ...
    if (type == GUNS_TYPE || type == ITEMS_TYPE) {
